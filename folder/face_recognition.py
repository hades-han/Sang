...
class FaceRecognition:
    def __init__(self, path='./'):
        ...
        self.face_recognition_model = pickle.load(
            open(os.path.join(path, 'ml_face_person_identity.pkl'), 'rb')
        )

        # Khởi tạo models
        self.detectorModel = cv2.dnn.readNetFromCaffe(
            self.faceDetectionProto, self.faceDetectionModel
        )
    ...
    def pipeline_model(self, frame):
        ...
        img_blob = cv2.dnn.blobFromImage(
            frame, 1, (300, 300), (104, 177, 123), swapRB=False, crop=False
        )
        ...
        for i, confidence in enumerate(detections[0, 0, :, 2]):
            if confidence > 0.5:
                ...
                individual_predictions = [
                    clf.predict(vectors)[0] for clf in self.face_recognition_model.estimators_
                ]
                agreement_count = sum(
                    [1 for pred in individual_predictions if pred == face_name]
                )
